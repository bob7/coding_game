...
for ta in range(0,len(list(set(leaf_nodes)))/2):
    bro[leaf_nodes[2*ta]]=leaf_nodes[2*ta+1]
    bro[leaf_nodes[2*ta+1]]=leaf_nodes[2*ta]

print "number of leafs: " + str(len(list(set(leaf_nodes))))
print ""
# print "brother of leaf "+ str(leaf_nodes[5]) + " is "+ str(bro[leaf_nodes[5]])


grabs = map(int,grabs)
print "Choose red leafs from: "+ " ".join(str(x) for x in grabs)
print ""
...
for i in red_leafs:
    g1.node(str(i),fillcolor='firebrick')

img_name='./'+foldername+'/gr'
...
# print ""
print "You chose "+ str(len(red_leafs)) + " red leafs (proportion  "+ str(len(red_leafs)) +"/"+str(len(leaf_nodes))+ " = "+ "{:.2f}".format(float(len(red_leafs))/float(len(leaf_nodes))) + "): " + " ".join(str(x) for x in red_leafs)
# str(map(int,red_leafs))
print ""
f.write("Red leafs are: "+ " ".join(str(x) for x in red_leafs))
f.write("\n")
f.write("\n")

###########brother section begins#################

# for Bred in red_leafs:
#     if bro[Bred] not in set(red_leafs):
#         Bcov = bro[Bred]
#         la=0
...
# for Bred in list(set(red_leafs)-set(red_leafs_d)):
while len(list(set(red_leafs)-set(red_leafs_d)))>0:
    print "Eligible positions table for remaining red: "
    print ""
    f.write("Eligible positions table for remaining red: ")
    f.write("\n")
    ...
    for tred in list(set(red_leafs)-set(red_leafs_d)):    
        ...
        f.write(str(tred).ljust(3) + ": " + " ".join(str(x) for x in da))
        f.write("\n")
    print "" 
    f.write("\n")
    
    cover_pair = map(int,raw_input("Input red leaf and you choice of cover for it, separated by a space: ").split())
    Bred = str(cover_pair[0])
    Bcov = str(cover_pair[1]) 
    
    print ""
    while label_test(Bcov,Bred)==0:
        da=map(int,eligible_choices(Bred))
        da.sort()
        print "Wrong choice - eligible choices for "+ str(Bred) +" are: " + " ".join(str(x) for x in da)
