...
# f.write("\n")
print ""
Bred = str(raw_input("Pick red leaf: "))
while not Bred=="0" and Bred not in grabs:
    Bred = str(raw_input("Wrong - pick red leaf: "))
print ""
g1.render('./'+foldername+'/g',cleanup=True)
...
while not Bred=="0" and not elmain==0 and not elsec==0:
    ...
    if not elmain==0 and Bred==sec_label[Bred]:
        Bcov = str(raw_input("Cover for main label: "))
        while label_test(Bcov,Bred)==0:
            Bcov = str(raw_input("Wrong choice - cover for main label: "))
    ...
    if not elmain==0 and not Bred==sec_label[Bred]:
        ...
        cover_pair = map(int,raw_input("Input covers for first and second label, separated by a space: ").split())
        Bcov = str(cover_pair[0])
        Bcov2 = str(cover_pair[1])
        while label_test(Bcov,Bred)==0 or label_test(Bcov2,Bred2)==0:
            cover_pair = map(int,raw_input("Wrong choice - input covers for first and second label, separated by a space: ").split())
            Bcov = str(cover_pair[0])
            Bcov2 = str(cover_pair[1])
