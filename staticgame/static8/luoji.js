...
let red_example = [328,257,378,407,260,410,324,373,266,317,275,367,432,438,432,361,353,424,380,325,395,350,394,291,373,431,412,377,414,302,255,368,391,374,385,432,395,262,271,443,411,375,388,260,386,267,331,391,268,362,312,338,372,351,402,277,277,383,265,424,344,290,367,322,375,362,273,355,309,331] 
let redList = []
let tmpList = []
...
function Blue_Step_1(d){  //This is the first step of the blue side. Click a red node.
    ...
    if(cc==1)
    {
        console.log("cc在blue_step_1中应该是0")
...
function Blue_Step_2(d) {   // this is blue player's turn
    ...
    if (cc == 0) {
        console.log("Blue_Step_2 的cc应该是1")
...
function Example(){
    ...
    for(var i = 0; i < red_example.length; i++){
        ...
        //Find the corresponding red node according to redList[i], 
        //and set its color attribute and phase attribute
        findNodeTo (tree, red_example[i]+"", node => {    // the label should be a string, so I make redList[i]+""
            node.firstStage = true
            node.red_end = false
            node.chosen = false
            node.blue = false
        ...
        Recording.push("r:" + red_example[i]) // recording the process
        cur_red_choice_num += 1   // Represents the actual number of choices in the Red player
    }
    console.log(Recording)
    if (cur_red_choice_num == Max_Choice_Num){
...
function Random(){
    ...
    diff = (endLabel - beginLabel + 1)
    nums = Math.floor(proporation * diff)
    // Math.floor(Math.pow(2,depth-1)*this.value/100)
    ...
    for(var i = 0; i < tmpList.length; i++){
        ...
        findNodeTo (tree, tmpList[i]+"", node => {    // the label should be a string, so I make redList[i]+""
            node.firstStage = true
            node.red_end = false
            node.chosen = false
            node.blue = false
